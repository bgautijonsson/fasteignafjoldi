...
```{r}
kaupskra <- read_csv2("kaupskra.csv") |> 
    semi_join(d,
              by = "sveitarfelag") |> 
...
kaupskra |> 
    mutate(utgdag = dmy_hm(utgdag),
           ar = year(utgdag)) |> 
    filter(byggar >= ar - 10) |> 
    select(sveitarfelag, ar, kaupverd) |> 
    mutate(kaupverd = cut(kaupverd/1000,
                          breaks = c(-Inf, 30, 40, 50, 60, 70, 80, 90, 100, Inf),
                          include.lowest = TRUE, right = FALSE)) |> 
    count(sveitarfelag, ar, kaupverd) |> 
    group_by(sveitarfelag, ar) |> 
    mutate(p = n / sum(n)) |> 
    ungroup() |> 
    filter(ar == 2021) |> 
    ggplot() +
    geom_ridgeline(aes(x = kaupverd, y = sveitarfelag, fill = sveitarfelag, height = p,
                       group = paste(sveitarfelag)), 
                   scale = 5, alpha = 0.4) +
    # facet_wrap("sveitarfelag", nrow = 2) +
    theme_half_open() +
    theme(legend.position = "none") +
...
plot_dat <- kaupskra |> 
    mutate(utgdag = dmy_hm(utgdag),
           ar = year(utgdag)) |> 
    filter(byggar >= ar - 3, ar >= 2014) |> 
    select(sveitarfelag, ar, kaupverd) |> 
    group_by(sveitarfelag, ar) |> 
    summarise(kaupverd = median(kaupverd * 1000),
              .groups = "drop") |> 
    group_by(sveitarfelag) |> 
    mutate(label = ifelse(ar == max(ar), sveitarfelag, NA_character_),
           x = ar,
...
plot_dat |> 
    ...
    geom_text(data = plot_dat |> group_by(sveitarfelag) |> filter(ar == max(ar)),
              aes(label = label, x = x, y = y, col = label), nudge_x = 0.05, hjust = 0, size = 4) +
    geom_rangeframe() + 
    scale_x_continuous(limits = c(2014, 2024.1),
                       breaks = 2014:2022) +
    scale_y_continuous(labels = label_number(accuracy = 1, suffix = " kr"),
                       breaks = c(range(plot_dat$kaupverd))) +
    scale_colour_brewer(type = "qual", palette = "Dark2") +
    theme_tufte() +
    theme(legend.position = "none") +
    labs(x = NULL,
         y = NULL,
         title = "Miðgildi kaupverðs nýrra íbúða (byggðar síðustu 3 ár) í sveitarfélögum Höfuðborgarsvæðisins")
```
...
plot_dat <- kaupskra |> 
    ...
    inner_join(visitala,
               by = "ar") |> 
    mutate(kaupverd = kaupverd / visitala_2022)
...
p <- plot_dat |> 
    ...
    labs(x = NULL, y = NULL, 
         title = "Dreifing kaupverðs nýrra íbúða í sveitarfélögum Höfuðborgarsvæðisins (byggt 2014 - 2022)", 
         subtitle = "Borið saman við dreifingu í öðrum sveitarfélögum Höfuðborgarsvæðisins. Leiðrétt fyrir vísitölu neysluverðs.",
         caption = "Kóði og gögn: https://github.com/bgautijonsson/fasteignafjoldi") + 
