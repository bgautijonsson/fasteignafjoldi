...
ggsave(plot = p, filename = "arleg_breyting_eignarhald_facet.png",
...
ggsave(plot = p, filename = "arleg_breyting_eignarhaldhlutf_facet.png",
       width = 8, height = 0.6 * 8, scale = 1.6, bg = "white")
```
...
p <- plot_dat |> 
    ...
    scale_x_continuous(breaks = c(range(d$ar), 2000, 2005, 2010, 2015)) +
    scale_y_continuous(labels = label_percent(accuracy = 1),
                       breaks = c(0, 0.1, 0.2, 0.3, 0.36, 0.4, 0.5),
                       limits = c(0, 0.5)) +
    # coord_cartesian(ylim = c(0.23, 0.38)) +
    theme_tufte() +
    theme(plot.title = element_text(face = "bold")) +
...
p <- plot_dat |> 
    ggplot(aes(ar, value)) +
    geom_area(aes(fill = name), position = "fill", alpha = 0.7) +
    scale_x_continuous(breaks = c(range(d$ar), 2000, 2005, 2010, 2015),
                       expand = expansion()) +
    scale_y_continuous(labels = label_percent(accuracy = 1),
...
plot_dat <- d |> 
    ...
    summarise(fjoldi = sum(fjoldi),
              .groups = "drop") |> 
    mutate(sveitarfelag = fct_reorder(sveitarfelag, fjoldi))
...
p <- plot_dat |> 
    ...
    labs(x = NULL,
         y = NULL,
         title = "Hreinn fjöldi nýbyggðra sér- og fjölbýliseigna frá 2018",
         subtitle = "Athugið að myndin er á lograkvarða: munurinn á 10, 100 og 1000 er sá sami.")

p

ggsave(plot = p, filename = "fjoldi_2018.png",
       width = 8, height = 0.5 * 8, scale = 1.7, bg = "white")
```
...
plot_dat <- d |> 
    ...
    summarise(fjoldi_hofdatolu = sum(fjoldi_hofdatolu),
              fjoldi = sum(fjoldi),
              .groups = "drop") |> 
    mutate(sveitarfelag = str_c(sveitarfelag, " (", fjoldi, ")"),
           sveitarfelag = fct_reorder(sveitarfelag, fjoldi_hofdatolu),
           x = ifelse(sveitarfelag == "Dalvíkurbyggð (28)", 18.2, NA_real_),
...
p <- plot_dat |> 
    ...
    labs(x = NULL,
         ...
         subtitle = "Sýnt fyrir sveitarfélög (og hreinan fjölda nýrra eigna) með fleiri en 1.000 íbúa árið 2022")
